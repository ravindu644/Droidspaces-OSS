...
/* Optional override for JNI/rootless mode (set by ds_set_workspace_dir) */
static char g_workspace_override[PATH_MAX];

void ds_set_workspace_dir(const char *path) {
  if (path)
    safe_strncpy(g_workspace_override, path, sizeof(g_workspace_override));
  else
    g_workspace_override[0] = '\0';
}

const char *get_workspace_dir(void) {
  if (g_workspace_override[0])
