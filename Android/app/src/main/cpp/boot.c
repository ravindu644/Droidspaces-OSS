...
int internal_boot(struct ds_config *cfg) {
   ...
   * We ALWAYS start with MS_PRIVATE because MS_SHARED breaks pivot_root
   * on some kernels. We will switch to MS_SHARED after the rootfs relocation
   * if requested. */
  ...
  if (setup_dev(".", cfg->hw_access, cfg->gpu_mode, cfg->privileged_mask,
                 cfg->rootless) < 0) {
  ...
  /* 17. pivot_root into the new rootfs. */
  ...
  setup_devpts(cfg->hw_access, cfg->rootless);
  ...
  /* 21. Cleanup .old_root from pivot_root. */
