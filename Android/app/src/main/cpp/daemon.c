...
int ds_client_run(int argc, char **argv) {
  ...
  if (interactive && !has_tty) {
    ...
    if (forces_tty) {
      if (is_enter) {
         ...
         * Save the modified entries so we can restore them if the daemon is
         * unreachable (the caller's argv must not be corrupted on return -2). */
        saved_argc = argc;
        for (int i = 0; i < argc; i++) {
  ...
  /* Daemon connected — restore argv in case the caller continues
   * (the early -2 path, handled below via restore-and-fail). */
  if (saved_last) {
    argv[argc] = saved_last;
    argc = saved_argc;
  }
...
fail:
  /* Restore argv modified by foreground stripping above.
   * The caller's argv must not be corrupted when we return -2. */
  if (saved_last) {
    argv[argc] = saved_last;
    argc = saved_argc;
  }
