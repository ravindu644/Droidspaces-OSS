   ...
   */
  {
    ...
    if (access("/proc/sys", F_OK) == 0) {
      if (mount("/proc/sys", "/proc/sys", NULL,
        ...
        /* Single-step remount failed; try bind first then remount RO */
...
int setup_dev(const char *rootfs, int hw_access, int gpu_mode,
              int privileged_mask, int rootless) {
...
int setup_devpts(int hw_access, int rootless) {
  const char *pts_path = "/dev/pts";

  /* Rootless: bind-mount host /dev/pts (newinstance requires root) */
  if (rootless) {
    mkdir(pts_path, 0755);
    if (mount("/dev/pts", pts_path, NULL, MS_BIND | MS_REC, NULL) == 0)
      return 0;
    ds_warn("Failed to bind-mount host /dev/pts: %s", strerror(errno));
    return -1;
  }
...
int check_volatile_mode(struct ds_config *cfg) {
  ...
  /* Rootless: OverlayFS inside user namespaces requires kernel 5.11+ */
  if (cfg->rootless) {
    struct utsname uts;
    int maj = 0, min = 0;
    if (uname(&uts) == 0)
      sscanf(uts.release, "%d.%d", &maj, &min);
    if (maj < 5 || (maj == 5 && min < 11)) {
      ds_error("Volatile mode in rootless requires kernel 5.11+ (detected "
               "%d.%d).", maj, min);
      return -1;
    }
  }
...
int mount_rootfs_img(const char *img_path, char *mount_point, size_t mp_size,
  ...
  for (int attempt = 0; attempt < 3; attempt++) {
     ...
     * auto-clear, but be explicit to cover edge cases. */
    if (loop_fd >= 0)
