...
int start_rootfs(struct ds_config *cfg) {
  ...
  if (cfg->rootfs_img_path[0]) {
    /* Rootless: image mode not supported (loop mount requires root) */
    if (cfg->rootless) {
      ds_error("Rootless mode does not support rootfs images. Use a directory "
               "rootfs (-r /path/to/rootfs).");
      goto cleanup;
    }
  ...
  /* Rootless: pipes for NEWUSER handshake (monitor→parent, parent→monitor) */
  int user_pipe[2] = {-1, -1}, ack_pipe[2] = {-1, -1};
  if (cfg->rootless) {
    if (pipe(user_pipe) < 0 || pipe(ack_pipe) < 0) {
      ds_error("pipe failed for rootless handshake: %s", strerror(errno));
      close(sync_pipe[0]); close(sync_pipe[1]);
      goto cleanup;
    }
    fcntl(user_pipe[0], F_SETFD, FD_CLOEXEC);
    fcntl(user_pipe[1], F_SETFD, FD_CLOEXEC);
    fcntl(ack_pipe[0], F_SETFD, FD_CLOEXEC);
    fcntl(ack_pipe[1], F_SETFD, FD_CLOEXEC);
  }
  ...
  if (monitor_pid == 0) {
     ...
     * Only enable cgroupns when V2 is active. */
    int cg_ns_ok = !cfg->rootless &&
                   (access("/proc/self/ns/cgroup", F_OK) == 0) &&
                   ds_cgroup_host_is_v2();
    if (cg_ns_ok) {
    ...
    /* Rootless: unshare NEWUSER first, handshake with parent for uid/gid maps,
     * then continue with the rest of the namespace unshares. */
    if (cfg->rootless) {
      if (unshare(CLONE_NEWUSER) < 0)
        ds_die("unshare(CLONE_NEWUSER) failed: %s", strerror(errno));

      /* Signal parent that we've unshared NEWUSER */
      char c = 0;
      if (write(user_pipe[1], &c, 1) != 1)
        _exit(EXIT_FAILURE);

      /* Wait for parent to write uid/gid maps */
      if (read(ack_pipe[0], &c, 1) != 1)
        _exit(EXIT_FAILURE);
  ...
  /* Rootless: wait for monitor to unshare NEWUSER, then write uid/gid maps */
  if (cfg->rootless) {
    uid_t real_uid = getuid();
    gid_t real_gid = getgid();
    char map[64];

    /* Wait for monitor to signal that it has unshared NEWUSER */
    char c = 0;
    if (read(user_pipe[0], &c, 1) != 1) {
      ds_error("Rootless handshake: monitor failed to signal");
      close(user_pipe[0]); close(ack_pipe[1]);
      goto cleanup;
    }

    /* Write uid_map: map real UID to root (0) inside userns */
    snprintf(map, sizeof(map), "0 %u 1", real_uid);
    char uid_path[64];
    snprintf(uid_path, sizeof(uid_path), "/proc/%d/uid_map", monitor_pid);
    if (write_file(uid_path, map) < 0) {
      ds_error("Failed to write uid_map: %s", strerror(errno));
      close(user_pipe[0]); close(ack_pipe[1]);
      goto cleanup;
    }

    /* setgroups: deny (required before writing gid_map without CAP_SETGID) */
    snprintf(uid_path, sizeof(uid_path), "/proc/%d/setgroups", monitor_pid);
    write_file(uid_path, "deny"); /* best-effort */

    /* Write gid_map: map real GID to root (0) inside userns */
    snprintf(map, sizeof(map), "0 %u 1", real_gid);
    snprintf(uid_path, sizeof(uid_path), "/proc/%d/gid_map", monitor_pid);
    if (write_file(uid_path, map) < 0) {
      ds_error("Failed to write gid_map: %s", strerror(errno));
      close(user_pipe[0]); close(ack_pipe[1]);
      goto cleanup;
    }

    /* Acknowledge monitor so it can continue */
    if (write(ack_pipe[1], &c, 1) != 1) {
      ds_error("Rootless handshake: ack failed");
      close(user_pipe[0]); close(ack_pipe[1]);
      goto cleanup;
    }
   ...
   * Skip in rootless mode (mount requires root). */
  if (is_android() && !cfg->rootless && !cfg->rootfs_img_path[0] && !is_ramfs("/"))
...
int enter_namespace(pid_t pid, struct ds_config *cfg) {
  ...
  /* User namespace must be entered FIRST so capabilities take effect
   * before other namespace setns() calls. It is the first entry. */
  const char *ns_names[] = {"user", "mnt", "uts", "ipc", "pid", "cgroup", "net"};
  int ns_fds[7];
  char path[PATH_MAX];
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (ns_fds[i] < 0) {
      if (i == 0) { /* user namespace — warn but continue; mnt is the real hard
                       requirement. If user ns fails, mnt will likely fail too. */
        ds_warn("Failed to open user namespace at %s: %s", path,
                strerror(errno));
        /* Don't return early — let the loop try mnt and give a better error */
        continue;
      }
      if (i == 1) { /* mnt is mandatory */
        ds_error("Failed to open mount namespace at %s: %s", path,
                 strerror(errno));
        /* Cleanup previous fds */
        for (int j = 1; j < i; j++)
          if (ns_fds[j] >= 0) close(ns_fds[j]);
        if (ns_fds[0] >= 0) close(ns_fds[0]);
        return -1;
      }
      if (errno != ENOENT && i != 6) {
  ...
  /* 2. Enter namespaces (user ns FIRST, then the rest) */
  for (int i = 0; i < 7; i++) {
    if (ns_fds[i] < 0)
      continue;

    /* Skip entering the 'net' namespace (index 6) if host networking is enabled
     */
    if (i == 6 && cfg && cfg->net_mode == DS_NET_HOST) {
      close(ns_fds[i]);
      continue;
    }
    ...
    if (setns(ns_fds[i], 0) < 0) {
      ...
      if (i == 0) {
        /* User namespace: EPERM expected in rootless mode if the caller
         * doesn't have CAP_SYS_ADMIN in the target userns.  Other namespaces
         * may also fail as a consequence — log and continue. */
        ds_warn("setns(user) failed: %s. Other namespaces may also fail.",
                strerror(errno));
      } else if (i != 6) {
...
int enter_rootfs(struct ds_config *cfg, const char *user) {
  ...
  if (child == 0) {
    ...
    ds_log_silent = 1;
    ds_seccomp_apply_minimal(cfg->hw_access, cfg->privileged_mask, cfg->rootless);
    android_seccomp_setup(0);
...
int show_info(struct ds_config *cfg, int trust_cfg_pid) {
  ...
  if (cfg->format_output) {
    ...
    if (access(DS_SELINUX_ENFORCE_PATH, R_OK) == 0) {
      printf("SELINUX=%s\n",
