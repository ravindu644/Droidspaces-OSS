 ...
 * cgroup2. */
void ds_cgroup_host_bootstrap(int rootless) {
  /* Rootless: no privilege to mount cgroup2 on the host */
  if (rootless) {
    ds_log("[CGROUP] Skipping host bootstrap (rootless mode).");
    return;
  }
...
int setup_cgroups(int is_systemd) {
  ...
  int v2_active = ds_cgroup_host_is_v2();
  ...
  if (v2_active) {
    ...
    /* V1 fallback: synthesize fresh mounts for all controllers. */
...
int ds_cgroup_apply_limits(struct ds_config *cfg) {
  ...
  /* Rootless: no cgroup write access. Limits can only be applied if the
   * cgroup subtree is delegated to the user (requires cgroup manager). */
  if (cfg->rootless) {
    ds_warn("[CGROUP] Resource limits not supported in rootless mode.");
    cfg->memory_limit = 0;
    cfg->cpu_quota = 0;
    cfg->pids_limit = 0;
    return 0;
   ...
   * by the host systemd and cannot be reliably delegated. Skip when the
   * host has no v2 mount. */
  if (!ds_cgroup_host_is_v2()) {
    cfg->memory_limit = 0;
    cfg->cpu_quota = 0;
    cfg->pids_limit = 0;
    return 0;
