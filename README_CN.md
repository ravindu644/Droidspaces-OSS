...
> [!TIP]
> 查看[社区支持的 Android 设备](./Documentation/community-supported-devices.md)，了解已知可运行 Droidspaces 的手机列表。
...
- [使用指南](#usage)
- [疑难解答](./Documentation/Troubleshooting.md)
...
| **深度 Android 集成** | 支持两种守护进程模式：**原生 init.rc**（最底层的集成方式，自动生成/不可被终止的持久性）和**用户空间守护进程**（应用内可切换，通过 `post-fs-data.sh` 启动，无需修改镜像）。**两种模式均绕过 root 域 seccomp 限制，确保容器生命周期稳定** [[init.rc 开发者指南](./init/README.md)]。 |
...
| **Linux GPU 加速** | 在 Linux 桌面端上无需额外配置 AMD 和 Intel GPU 硬件加速。[[更多信息](./Documentation/GPU-Acceleration.md)] |
...
| **Rootfs 镜像 / 直接块设备支持** | 支持从 ext4 `.img` 文件启动容器，支持自动 loop 挂载、文件系统检查以及 SELinux 上下文加固（如需要）。CLI 模式下也支持挂载分区、SD 卡等块设备！**Android 应用同样支持以 rootfs.img 模式创建可移植容器** [ [如何手动创建 ext4 rootfs.img？](./Documentation/Installation-Linux.md#option-b-create-an-ext4-image-recommended)] |
...
| **进程隔离** | **完全支持** | 完全支持 | 完全支持 | 无（共享宿主机 PID 命名空间） | 无（共享宿主机 PID 命名空间） |
| **文件系统隔离** | **完全支持** | 完全支持 | 完全支持 | 无（`chroot /proc/1/root /system/bin/sh` 可瞬间逃逸） | 有限 |
| **挂载隔离** | **完全支持** | 完全支持 | 完全支持 | 无 | 无 |
| **IPC / UTS / Cgroup 隔离** | **完全支持** | 完全支持 | 完全支持 | 无 | 无 |
| **Android 端容器持久性** | **真正不可杀死。存活超过 15 天。完全免疫开发者选项中的"不保留活动"和"无后台进程"。** | 低（被 Android LMK / 电池优化杀死） | 低（被 Android LMK / 电池优化杀死） | 低（被 Android LMK / 电池优化杀死） | 低（被 Android LMK / 电池优化杀死） |
...
| **硬件与原生 GPU 直通** | **完全支持（一键开关）。Adreno Turnip、USB、传感器、网络接口、块设备。完整的 `systemd-udevd` 支持 — 如同真正的 Linux PC。** | 手动绑定挂载，无 udev | 手动绑定挂载，无 udev | 手动绑定挂载，无 udev | 无 |
| **Termux-X11 支持** | **完全支持（一键开关）** | 手动套接字透传 | 手动套接字透传 | 手动套接字透传 | 手动套接字透传 |
| **特权模式** | **完全支持 + 可定制（`--nomask`、`--nocaps`、`--noseccomp` 等）** | 手动配置 | 支持（`--privileged`） | 完全支持（无任何防护） | 不支持 |
...
| **旧版内核支持 (3.10+)** | **完全支持** | 不稳定（cgroup 冲突，在许多旧 Android 内核上不可用） | 不稳定（cgroup 冲突，在许多旧 Android 内核上不可用） | N/A | N/A |
...
| **二进制大小** | **每种架构约 400KB** | 10MB+ | 50MB+ | N/A | 约 10MB |
