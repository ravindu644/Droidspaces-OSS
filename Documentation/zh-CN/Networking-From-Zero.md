...
- [第四部分：DNS（名称如何变成地址）](#第四部分dns名称如何变成地址)
...
- [第七部分：网络命名空间（容器如何隔离）](#第七部分网络命名空间容器如何隔离)
...
- [第九部分：全新的网关模式——融会贯通](#第九部分全新的网关模式融会贯通)
    ...
    - [为什么 bridge-nf-call-iptables 设为 0](#为什么-bridge-nf-call-iptables-设为-0)
    - [网关容器必须先运行](#网关容器必须先运行)
    - [容器停止时会发生什么](#容器停止时会发生什么)
- [第十部分：网关模式标志与配置](#第十部分网关模式标志与配置)
    - [必选 vs 可选标志](#必选-vs-可选标志)
...
之所以叫"局域"，是因为设备在物理上很近：你的手机、笔记本电脑、智能电视，都连接着你的家庭 Wi-Fi 路由器。它们都生活在同一个局域网内，可以直接相互通信。
...
**规则：** 局域网上的每台设备都配置了一个"默认网关"——当不知道往哪发送时，所有流量都发送到的那个地址。
...
DHCP 服务器：   "我听到了。来，拿 192.168.1.42。另外，你的网关是 192.168.1.1，
...
在你的家里，**路由器运行着 DHCP 服务器**。它为每一台连接的设备分配 IP。
...
IP 地址很难记忆。没有人会输入 `142.250.80.46` 来访问 Google。人们输入 `google.com`。
...
Droidspaces 运行一个小型的每容器 DHCP 服务器，监听在容器的宿主机侧 veth 上。整个 `172.28.0.0/16` 子网属于 Droidspaces（`172.28.0.x` 段保留给网关自身，所以容器始终落在 `172.28.1.x` 到 `172.28.254.x` 之间），其中所有内容通过 Android 的真实接口进行 NAT 出站。
...
### 为什么 bridge-nf-call-iptables 设为 0
