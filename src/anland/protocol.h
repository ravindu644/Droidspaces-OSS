...
struct InputEvent {
  ...
  union {
    ...
    struct {
      uint32_t size; // 这个packet只是通知包
                     // 作为header真正数据会集中发送,这里通知随后数据的大小
    } clipboard;
    struct {
      uint32_t size; // 这个packet只是通知包
                     // 作为header真正数据会集中发送,这里通知随后数据的大小
 ...
 * blocks input/clipboard. The socket is full duplex: the producer writes
 * desktop playback PCM that the consumer reads, and the consumer writes
 * microphone PCM that the producer reads -- each side only ever reads what the
 * other wrote. Every direction sends one AUDIO_MSG_FORMAT, then a stream of
 * AUDIO_MSG_PCM.
 *
 ...
 * device actually chose, and sends one AUDIO_MSG_FORMAT per direction (tagged
 * with role) to the producer. The producer builds its PipeWire sink/source to
 * match, so neither side resamples blindly and the PCM byte math lines up on
 * both ends.
 */
...
#define AUDIO_ROLE_PLAYBACK                                                    \
  0 /* producer -> consumer (desktop sound -> speaker) */
#define AUDIO_ROLE_CAPTURE                                                     \
  1 /* consumer -> producer (mic -> virtual source)    */
...
struct audio_format {
  ...
  uint32_t quantum;  /* requested buffer in frames (latency preset); 0 = engine
                        default */
} __attribute__((packed));
